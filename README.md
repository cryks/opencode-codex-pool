...
- Shows a compact selection toast immediately before each outbound prompt attempt, with a `>` marker on the chosen account, a stale-cache age tag like `(5m ago)` when ranking is using reused usage data, whether fast-mode is enabled for that attempt, per-account routing summaries that show the final score plus `[main window]` and `[guard window]` details when a multi-window `rate_limit` is reduced, the reason that account was chosen, and a compact fast-mode section that shows `status  enabled` or `status  disabled (<reason>)` plus a `main` line and `= base`, optional `- guard`, and `= final` breakdown.
