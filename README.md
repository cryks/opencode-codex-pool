...
After the account is chosen, the fetch layer may decorate that attempt with `service_tier: "priority"`. This is intentionally post-ranking: routing still uses the existing quota score, then fast-mode checks the selected account's in-memory usage cache. A usage snapshot is considered fresh for 60 seconds; after that it may still be reused for fast-mode and toast decisions while a background refresh is started. The trigger is conservative: for every complete window in every rate limit, it computes `remaining_capacity = 1 - used_percent / 100` and `remaining_time = reset_after_seconds / limit_window_seconds`, then enables priority only when the minimum `remaining_capacity - remaining_time` is at least `0.1`. Caller-provided `service_tier` or `serviceTier` always wins, and each retry/failover rebuilds its body from the immutable base snapshot so one account's tier choice never leaks into another attempt. The account-selection toast includes a `Fast-mode enabled` or `Fast-mode disabled` line for the current attempt, marks the selected account with `>` in the `Account:`/`Accounts:` list, and is emitted immediately before the outbound prompt request for that account. When sticky affinity keeps the same account but a later attempt re-evaluates to a different fast-mode state, the router emits a separate fast-mode toast before that prompt attempt.
