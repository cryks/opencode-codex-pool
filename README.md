...
- Shows a compact selection toast immediately before each outbound prompt attempt, with a `>` marker on the chosen account, a stale-cache age tag like `(5m ago)` when ranking is using reused usage data, per-account routing summaries that show the final score plus `[main window]` and `[guard window]` details when a multi-window `rate_limit` is reduced, the reason that account was chosen, and a one-line fast-mode summary such as `Fast: enabled (+0.806)` or `Fast: disabled (cap<3%, rate.primary)`. When guard debt changes the result, that summary expands inline to include the main window score and guard cost.
