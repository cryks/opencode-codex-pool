...
- The feature uses the selected account's in-memory usage cache. Fresh usage is authoritative for 60 seconds; after that, stale in-memory usage may still enable fast-mode for the current attempt while a background refresh is started. The shared SQLite score cache remains routing-only.
